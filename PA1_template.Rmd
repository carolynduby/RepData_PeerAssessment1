...
```
Downloaded activity data downloaded from url `r source_url` on `r download_time`
...
**Steps per day(mean - NAs removed) = `r mean_steps_per_day_na_removed`** 

**Steps per day(median - NAs removed) = `r median_steps_per_day_na_removed`** 
...
**Interval with maximum average number of steps (NAs removed) = `r maximum_avg_step_interval$interval`**

**Maximum average number of steps (NAs removed) = `r maximum_avg_step_interval$avgstepsperinterval`**
...
**Number of NA values in the original data set = `r na_sum`**
...
**Steps per day(mean - NAs imputed) = `r mean_steps_per_day_na_imputed`** 

**Steps per day(median - NAs imputed) = `r median_steps_per_day_na_imputed`** 
...
Imputed mean steps per day are `r mean_impact` than mean steps per day with NAs removed.
Difference between means = `r mean_difference`.
Imputed median steps per day are `r median_impact` than median steps per day with NAs removed.
Difference between medians = `r median_difference`.
